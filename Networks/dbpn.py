...
class Net_stage1(nn.Module):
    ...
    def forward(self, ms, pan):
        ...
        # x_f1, l1, h1 = self.DBPNnet1(x) ##[8,16,264,264]
        # x_f2, l2, h2 = self.DBPNnet2(x_f1)
        # x_f3, l3, h3 = self.DBPNnet3(x_f2)
        xtop, xbot, concat1_l, concat1_h, concat3_l, concat3_h = self.DBPNnet(x1ltop, x1rtop, x1lbot, x1rbot)
...
class Net_stage12(nn.Module):
    def __init__(self, num_channels=8, in_c=8, n_feat=28, base_filter=28, num_stages=3,scale_factor=4,
                 scale_feat=28, kernel_size=3, reduction=4, bias=False):
        super(Net_stage12, self).__init__()
        self.up = nn.Upsample(scale_factor=4, mode='bicubic')
        act = nn.PReLU()
        modules_body1 = []
        ...
        self.conv2_2 = nn.Sequential(conv(n_feat, in_c, kernel_size, bias=bias), nn.Tanh())
    def forward(self, ms, pan):
        _,_,H,W = ms.shape
        ms2top_img = ms[:,:,0:int(H/2)+4,:]
        ms2bot_img = ms[:,:,int(H/2)-4:H,:]
...
class DBPN_block1(nn.Module):
    ...
    def forward(self, xltop, xrtop, xlbot, xrbot):
        ...
        concat_hbot = torch.cat((concat_hlbot[:, :, :, :w - 32], yhb,
                                 concat_hrbot[:, :, :, 32:]), 3)
        # print(concat_lltop[:, :, :, :w // 4 - 8].shape, ylt.shape, concat_lrtop[:, :, :, 8:].shape)
        ...
        for i in range(32):
            ...
            if i==0:
                ...
                yhb = torch.cat((yhb, h1lbot[:, :, :, W - 32 + i
                    :W - 31 + i] * y1 + h1rbot[:, :, :, i:i + 1] * (1 - y1)), 3)
        # print(ylt.shape)
        l1top = torch.cat((l1ltop[:,:,:,:W//4-8], ylt,
                          l1rtop[:,:,:,8:]), 3)
        l1bot = torch.cat((l1lbot[:, :, :, :W//4-8], ylb,
                           l1rbot[:, :, :, 8:]), 3)
        h1top = torch.cat((h1ltop[:, :, :, :W-32], yht,
                           h1rtop[:, :, :, 32:]), 3)
        h1bot = torch.cat((h1lbot[:, :, :, :W-32], yhb,
                           h1rbot[:, :, :, 32:]), 3)
